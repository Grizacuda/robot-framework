...
This container can be run using the following command:
    ...
    docker run \
        ...
        grizacuda/robot-framework:<version>
...
It is possible to parallelise the execution of your test suites. Simply define the `ROBOT_THREADS` environment variable, for example:
    ...
    docker run \
        -e ROBOT_THREADS=4 \
        grizacuda/robot-framework:latest
    ...
    pipeline {
        ...
        stages {
            stage('Functional regression tests') {
                steps {
                    sh "docker run --shm-size=1g -e BROWSER=firefox -v $WORKSPACE/robot-tests:/opt/robotframework/tests:Z -v $WORKSPACE/robot-reports:/opt/robotframework/reports:Z grizacuda/robot-framework:latest"
                }
            }
        }
    }
...
The pipeline stage can also rely on a Docker agent, as shown in the example below:
    ...
    pipeline {
        ...
        stages {
            stage('Functional regression tests') {
                agent { docker {
                    image 'grizacuda/robot-framework:latest'
